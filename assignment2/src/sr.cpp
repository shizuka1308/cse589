...
int checksum(struct pkt packet)
{
  ...
  int localchecksum = 0;
  int iterator = 0;
  while (iterator < CHUKSZE && data[iterator] != '\0')
  {
    localchecksum += data[iterator];
    iterator = iterator + 1;
  }
...
void A_output(struct msg message)
{
  ...
  if (ready_active == 0)
  {
    last = packets.at(sequence);
    tolayer3(HOST_A, last);
...
{
  for (int iterator = 0; iterator < times.size(); iterator++)
  {
    timeout = get_sim_time() - times.at(iterator);
    if (timeout >= RTT)
    {
      tolayer3(HOST_A, last);
      starttimer(HOST_A, RTT);
